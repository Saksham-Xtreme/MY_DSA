...
void solveMaze(int maze[][4],int x,int y,string sol,int n,vector<vector<bool>> vis){
    ...
    if(x-1>=0 && !vis[x-1][y] && maze[x-1][y]==1 ){
        vis[x][y]=true;
        solveMaze(maze,x-1,y,sol+"U",n,vis);
        vis[x][y]=false;
    ...
    if(x+1<n && !vis[x+1][y] && maze[x+1][y]==1 ){
        vis[x][y]=true;
        solveMaze(maze,x+1,y,sol+"D",n,vis);
        vis[x][y]=false;
    ...
    if(y+1<n && !vis[x][y+1] && maze[x][y+1]==1 ){
        vis[x][y]=true;
        solveMaze(maze,x,y+1,sol+"R",n,vis);
        vis[x][y]=false;
    ...
    if(y-1>=0 && !vis[x][y-1] && maze[x][y-1]==1 ){
        vis[x][y]=true;
        solveMaze(maze,x,y-1,sol+"L",n,vis);
        vis[x][y]=false;
